...
while True:
    ...
    # Around 10 percent users churn out every time
    No_of_users_churned = int(len(Currently_Being_used_phn_nos)/10)
    Users_Churned = []

    random.shuffle(Currently_Being_used_phn_nos)
    for i in range(No_of_users_churned):
        Users_Churned.append(Currently_Being_used_phn_nos[i])

    random.shuffle(Currently_Being_used_phn_nos)
    #  ----------------- Writing the CDR data ------------------------------
    header_for_CDR = ['Phone Number', 'Day Mins', 'Int Mins', 'CustServ Calls', 'Churn']
    ...
    for i in range(len(Currently_Being_used_phn_nos)):
        ...
        CDR_Data[i].append(round(random.uniform(0.0, 60.00), 1))
        CDR_Data[i].append(random.randint(0, 10))

        if Currently_Being_used_phn_nos[i] in Users_Churned:
            CDR_Data[i].append("TRUE")
        else:
            CDR_Data[i].append("FALSE")
    ...
    # Remove the churned users
    for i in range(No_of_users_churned):
        Currently_Being_used_phn_nos.pop(i)
    # -------------------- CDR Data Written ---------------------------------------------
    ...
    # 3 to 1o ussers will be added in every iteration
